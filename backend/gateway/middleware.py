...
from common.redis_client import is_blacklisted, incr_rate_limit
...
class RequestLogMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        try:
            ...
            logger.info(
                "%s %s -> %s (%.2fms)", method, path, status_code, duration
            )
...
class JWTAuthMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        if not auth_header or not auth_header.startswith("Bearer "):
            await _send_json(
                send, 200, error_response(ErrorCode.UNAUTHORIZED, "缺少有效的认证信息")
            )
            return
        ...
        if not payload:
            await _send_json(
                send, 200, error_response(ErrorCode.UNAUTHORIZED, "Token 无效或已过期")
            )
            return
        ...
        if await is_blacklisted(token):
            await _send_json(
                send, 200, error_response(ErrorCode.UNAUTHORIZED, "Token 已失效，请重新登录")
            )
            return
        ...
        if user_id is None:
            await _send_json(
                send, 200, error_response(ErrorCode.UNAUTHORIZED, "Token 缺少用户信息")
            )
            return
...
class RateLimitMiddleware:
    """
    限流中间件（最内层）：
    - 仅对 POST /api/chat/sessions/{id}/messages 限流
    - 从 X-User-Id 获取用户 ID（JWT 中间件已注入）
    - Redis rate_limit:{user_id} INCR，TTL 60s
    - 超过上限返回 429
    """
    ...
    async def __call__(self, scope, receive, send):
        ...
        if not user_id:
            # 无用户标识，放行（JWT 中间件应当已拦截）
            await self.app(scope, receive, send)
            return

        # 限流计数
        try:
            count = await incr_rate_limit(int(user_id))
        except Exception as e:
            logger.warning("限流计数失败(放行): user_id=%s, err=%s", user_id, e)
            await self.app(scope, receive, send)
            return
        ...
        if count > limit:
            logger.warning(
                "触发限流: user_id=%s, count=%s, limit=%s",
                user_id, count, limit,
            )
            await _send_json(
                send, 200,
