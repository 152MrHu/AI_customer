...
from user_service.schemas.user import RegisterRequest, LoginRequest
...
async def login(data: LoginRequest) -> dict:
    ...
    user = await user_repo.find_by_account(account)
    if not user:
        raise BusinessError(
    ...
    # 登录失败次数校验
    fail_count = await get_login_fail_count(account)
    ...
    if not verify_password(data.password, user["password_hash"]):
        cur_count = await incr_login_fail(account)
        logger.warning(
            "登录失败: account=%s, fail_count=%s", account, cur_count
        )
        raise BusinessError(
            ErrorCode.ACCOUNT_OR_PASSWORD_ERROR, "账号或密码错误"
        )
    ...
    await clear_login_fail(account)
    await user_repo.update_last_login(user["user_id"])

    # 签发 JWT
    token = create_token(
        user["user_id"], user["username"], user["role"], data.remember_me
    )
    expires_in = get_token_ttl_seconds()
    logger.info(
        "用户登录成功: user_id=%s, username=%s", user["user_id"], user["username"]
    )
    return {
        "token": token,
...
async def list_users(
    ...
    items = await user_repo.list_users(
        keyword, status, page_params.offset, page_params.limit
    )
    total = await user_repo.count_users(keyword, status)
    return {
        "total": total,
